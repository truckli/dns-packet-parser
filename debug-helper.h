...
#define PRINT_STR(u) { printf("%s(line %d): %s = %s\n", __FUNCTION__, __LINE__, #u, u); }
#define SHOW(x) cout << #x << "=" << dec << x << "; "
#define SHOWHEX(x) cout << #x << "=" << hex << (int)x << "; "
#define SHOWHBUF(start, len) { cout << #start << "="; for (int i = 0; i < len; ++i) cout << hex << (int)start[i] << " "; cout << ";";}
#define test_print_match_val(x, val) { if (x == val) cout << #val << "; "; }
